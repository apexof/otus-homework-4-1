...
describe("Payments", function() {
  ...
  it("should be deployed", async function() {
    ...
    const address1 = payments.target
    const address2 = await payments.getAddress()
    console.log('address1',address1 );
    console.log('address2',address2 );
    expect(payments.target).to.be.properAddress;
    // console.log('addr',await payments.getAddress() );
  });
  ...
  it("should be possible to send funds", async function() {
    ...
    // );
    // const currentBlock = tx.blockNumber ? await ethers.provider.getBlock(1) : null;
    const currentBlock = await tx.getBlock()
